...
def tables(request,page=1):
    if request.method == 'GET':
        ...
        fault_number = DomainName.objects.filter(~Q(status_id=100) & Q(check_id=0) & Q(cert_valid_days=None)).count()
...
def tables_fault(request):
    if request.method == 'GET':
        project_all = Project.objects.all()
        domainall = DomainName.objects.filter(~Q(status_id=100) & Q(check_id=0) & Q(cert_valid_days=None))
        fault_number = DomainName.objects.filter(~Q(status_id=100) & Q(check_id=0) & Q(cert_valid_days=None)).count()
        Not_check_number = DomainName.objects.filter(check_id=1).count()
        lt_30 = DomainName.objects.filter(cert_valid_days__lt=30).count()
        data = {
